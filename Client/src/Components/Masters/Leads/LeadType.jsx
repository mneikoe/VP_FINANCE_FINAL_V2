...
const LeadType = () => {
  ...
  const { LeadType, loading, success } = useSelector((state) => state.LeadType);
  ...
  const showModal = (item = null) => {
    if (item) {
      setEditingItem(item);
      form.setFieldsValue({ leadType: item.leadType });
    } else {
      setEditingItem(null);
      form.resetFields();
    }
  ...
  const onFinish = async (values) => {
    if (editingItem) {
      const result = await dispatch(updateLeadType({ id: editingItem._id, data: values }));
      if (result.meta.requestStatus === "fulfilled") {
        toast.success("Lead source updated successfully!");
        loadData();
      }
    } else {
      const result = await dispatch(createLeadType(values));
      if (result.meta.requestStatus === "fulfilled") {
  ...
  const filteredData = (Array.isArray(LeadType) ? LeadType : []).filter((item) =>
    item.leadType?.toLowerCase().includes(searchText.toLowerCase())
  );
    ...
    <div style={{ padding: "24px", minHeight: "100vh", backgroundColor: "#f0f2f5" }}>
            ...
            <Row justify="space-between" align="middle" gutter={[16, 16]}>
              <Col xs={24} sm={12}>
                <Space align="center" size="middle">
                  <Title level={3} style={{ margin: 0 }}>Lead Source Management</Title>
                  <Badge count={filteredData.length} showZero color="#eb2f96" />
                </Space>
                <Text type="secondary">Manage the categories for lead classification</Text>
              </Col>
        ...
        title={
          <Space>
            ...
            <Title level={4} style={{ margin: 0 }}>
              {editingItem ? "Update Source" : "Create New Source"}
            </Title>
          </Space>
        }
        open={isModalOpen}
      ...
      >
        ...
        >
          ...
          >
            <Input 
              prefix={<UsergroupAddOutlined className="text-muted" />} 
              placeholder="e.g. Retail, Corporate, Affiliate" 
              size="large"
            />
          </Form.Item>
