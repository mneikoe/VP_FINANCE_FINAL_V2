  ...
  const updateStatus = async (id, newStatus) => {
    try {
      ...
      setCandidates(candidates.map(candidate =>
        candidate._id === id ? updatedCandidate.candidate : candidate
      ));
    } catch (error) {
          ...
          <tbody>
            {candidates.map((candidate) => (
              <tr key={candidate._id} style={{ textAlign: "center" }}>
                ...
                <td style={tdStyle}>
                  {candidate.resumeUrl ? (
                    <span style={{ color: '#007BFF', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => handlePreview(candidate.resumeUrl )}>
                      View Resume
                    </span>
                  ) : (
                    "N/A"
                ...
                <td style={tdStyle}>
                  <button onClick={() => deleteCandidate(candidate._id)} style={{ background: "red", color: "white", border: "none", padding: "5px 10px", borderRadius: "5px", cursor: "pointer" }}>
                    Delete
                  </button>
                </td>
                <td style={tdStyle}>
