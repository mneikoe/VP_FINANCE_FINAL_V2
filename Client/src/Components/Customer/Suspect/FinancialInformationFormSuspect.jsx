...
const FinancialInformationFormForSuspect = ({
  suspectId,
  suspectData,
  onSuspectCreated,
}) => {
  ...
  const handleSaveForm = (option, group) => {
    ...
    if (group === "investment") {
      ...
      if (
        ...
        toast.error(
          "Please fill in all required fields for the investment form"
        );
        return;
      }
      if (!formData.document) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      investmentForms.length === 0 &&
      loanForms.length === 0
    ) {
      toast.error(
        "Please save at least one form (insurance, investment, or loan) before submitting"
      );
      return;
    }
    ...
    try {
      const idToUse =
        suspectData && suspectData._id ? suspectData._id : suspectId;
      ...
      for (const form of insuranceForms) {
        ...
        await dispatch(
          addFinancialInfo({ suspectId: idToUse, financialData: formData })
        );
      }
  ...
  return (
    <Form onSubmit={handleSubmit} className="compact-financial-form">
        ...
        <div key={option} className="border p-3 mb-3">
          ...
          <Row>
            ...
            <Col md={4}>
              <Form.Group>
                <Form.Label>
                  Outstanding Amount <span className="text-danger">*</span>
                </Form.Label>
                <Form.Control
                  type="number"
      ...
      >
        Submit All (
        {insuranceForms.length + investmentForms.length + loanForms.length})
      </Button>
    </Form>
  );
};
