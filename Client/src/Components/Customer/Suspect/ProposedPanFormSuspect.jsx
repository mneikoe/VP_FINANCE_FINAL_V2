...
import { addProposedFinancialPlan } from "../../../redux/feature/SuspectRedux/SuspectThunx";
import { toast } from "react-toastify";

const ProposedPlanFormForSuspect = ({
  suspectId,
  suspectData,
  onSuspectCreated,
}) => {
    ...
    try {
      for (const plan of savedPlans) {
        ...
        const result = await dispatch(
          addProposedFinancialPlan({ suspectId: idToUse, formData })
        );
        if (result) {
          setSavedPlans([]);
          setPlans([
        ...
        <div className="mb-4">
          ...
          {savedPlans.map((savedPlan, index) => (
            <div key={index} className="border p-2 mb-2 bg-light">
              ...
              {savedPlan.documents.length} document(s)) - Status:{" "}
              {savedPlan.status}
            </div>
          ))}
        </div>
...
export default ProposedPlanFormForSuspect;
