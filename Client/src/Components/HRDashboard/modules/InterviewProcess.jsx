...
const InterviewProcess = () => {
  ...
  const fetchCandidates = async () => {
    try {
      setLoading(true);
      const response = await axios.get("/api/addcandidate/stage/Interview%20Process");
      setCandidates(response.data.candidates || []);
    } catch (error) {
      message.error("Failed to load interview candidates");
    } finally {
      setLoading(false);
    }
  };
        ...
        <Space>
          ...
          <Tooltip title="Select Candidate">
            <Button 
                ...
                onClick={() => updateStatus(record._id, "Selected")}
    ...
    <div className="fade-in">
      <Card bordered={false} style={{ borderRadius: 12, marginBottom: 24 }}>
        <Row justify="space-between" align="middle">
          <Col>
            <Title level={3} style={{ margin: 0 }}>Interview Process</Title>
            <Text type="secondary">Manage candidates in active interview stages</Text>
          </Col>
          <Col>
      ...
      <Modal
        title={<Space><UserOutlined style={{ color: '#1890ff' }} /> Interview Evaluation Profile</Space>}
        open={isModalOpen}
          ...
          <div className="fade-in">
            ...
            >
              ...
              <Descriptions.Item label="Interview Date">
                <Text strong color="blue">{selectedCandidate.interviewDate ? dayjs(selectedCandidate.interviewDate).format("DD MMM YYYY, hh:mm A") : "N/A"}</Text>
              </Descriptions.Item>
              ...
              <Descriptions.Item label="Interview Place" span={3}>
                <Text type="secondary">{selectedCandidate.interviewPlace || "Not Specified"}</Text>
              </Descriptions.Item>
            </Descriptions>
            ...
            <div style={{ marginTop: 24, textAlign: 'center' }}>
              <Space size="large">
                ...
                <Button 
                    ...
                    type="primary" 
                    icon={<CheckOutlined />} 
                    onClick={() => updateStatus(selectedCandidate._id, "Selected")}
