...
const InterviewProcess = () => {
  ...
  const fetchCandidates = async () => {
    try {
      ...
      const endpoints = [
        "/api/addcandidate/stage/Interview%20Process",
        "/api/addcandidate/status/Interview%20Process",
      ];
      ...
      // If no response succeeded, try getting all candidates and filter
      if (!response) {
        ...
        const interviewCandidates = allCandidates.filter((candidate) => {
          const currentStage = (candidate.currentStage || "")
            .toString()
            .toLowerCase()
            .trim();
          return currentStage === "interview process";
        });
      ...
      setError("Failed to load interview candidates");
      setCandidates([]);
    } finally {
      setLoading(false);
    }
  ...
  const updateStatus = async (candidateId, newStatus) => {
    try {
      ...
      // ✅ Change "Selected" to "Joining Data"
      const statusToUpdate =
        newStatus === "Selected" ? "Joining Data" : newStatus;

      const response = await axios.put(
        `/api/addcandidate/${candidateId}/stage`,
        {
          currentStage: statusToUpdate,
          currentStatus: statusToUpdate, // ✅ Also update currentStatus
          interviewDate: null,
        }
      );

      if (response.data && response.data.success) {
        alert(`Candidate moved to ${statusToUpdate}`);
        fetchCandidates();
    ...
    if (!candidates || candidates.length === 0) {
      return (
        <div className="text-center py-5">
          ...
          <p className="mb-4" style={{ color: "black" }}>
            Move candidates to interview from "Resume Shortlist" page
          </p>
          <button
            className="btn"
            onClick={fetchCandidates}
                      ...
                      <div className="d-flex gap-1">
                        ...
                        >
                          <FaCheck
                            ...
                            style={{ fontSize: "12px" }}
                          />
                          <span>Joining Data</span> {/* ✅ Text change */}
                        </button>
                        <button
                          className="btn btn-sm d-flex align-items-center justify-content-center flex-grow-1"
                          onClick={() =>
                            updateStatus(candidate._id, "Rejected")
                          }
                          style={{
                            backgroundColor: "white",
                            color: "black",
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="mb-0" style={{ color: "black" }}>
            ...
            {candidates && candidates.length !== 1 ? "s" : ""} in interview
          </p>
        </div>
        <button
                ...
                <div className="d-flex align-items-center">
                  ...
                  <div>
                    ...
                    <p className="mb-0" style={{ color: "black" }}>
                      Status:{" "}
                      <strong>
                        {candidate.currentStage || "Interview Process"}
                      </strong>
                    </p>
                  </div>
                </div>
          ...
          <div className="mt-4 pt-3 border-top">
            <div className="d-flex justify-content-between align-items-center">
              ...
              <div className="d-flex gap-2">
                ...
                >
                  <FaCheck className="me-2" style={{ fontSize: "14px" }} />
                ...
                >
                  <FaTimes className="me-2" style={{ fontSize: "14px" }} />
